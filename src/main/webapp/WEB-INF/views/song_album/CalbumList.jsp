...
<body>
	<center><h2>CLASSIC앨범 리스트</h2></center>
	<div class="container">
		<table class="table">
				...
				<td>
					<c:if test="${count>0 }">
						...
						<c:if test="${startPage > 10 }">
							<a href="CalbumList.do?pageNum=${startPage-10}">[이전]</a>
						</c:if>

						<%--페이지처리 --%>
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<a href="CalbumList.do?pageNum=${i}">[${i}]</a>
						</c:forEach>

						<%--다음 페이지 --%>
						<c:if test="${endPage < pageCount }">
							<a href="CalbumList.do?pageNum=${startPage+10}">[다음]</a>
						</c:if>
					</c:if>
				</td>
			</tr>
