				...
				<td>
					<c:if test="${count>0 }">
						...
						<c:if test="${startPage > 10 }">
							<a href="ETCalbumList.do?pageNum=${startPage-10}">[이전]</a>
						</c:if>

						<%--페이지처리 --%>
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<a href="ETCalbumList.do?pageNum=${i}">[${i}]</a>
						</c:forEach>

						<%--다음 페이지 --%>
						<c:if test="${endPage < pageCount }">
							<a href="ETCalbumList.do?pageNum=${startPage+10}">[다음]</a>
						</c:if>
					</c:if>
				</td>
			</tr>
