...
<body>
	...
	<form method="post" name="fb_writeForm" action="fb_writePro.do" onSubmit="return check()">
		...
		<div align="center">
			...
			<table width="1200px">
				...
				<tr>
					<td>제목: <input type="text" id="fb_title" name="fb_title"
						style="width: 600px;" placeholder="제목" value="${fb.fb_title}" /></td>
				</tr>
				<tr>
					<td>아이디: <input type="hidden" id="mem_id" name="mem_id"
						style="width: 170px;" maxlength="10" placeholder="아이디"
						value="${mem_id}" />${mem_id}</td>
				</tr>
				<tr>
					<td>닉네임: <input type="hidden" id="mem_nick" name="mem_nick"
						style="width: 170px;" maxlength="10" placeholder="닉네임"
						value="${mem_nick}" />${mem_nick}</td>
				</tr>

				<tr>
					<td>카테고리 <c:set var="fb_category" value="${fb_category }" /> <c:choose>
							...
							<c:when test="${fb_category eq 'media'}">
								<c:set var="fb_category" value="축구&풋살 동영상"/>
							</c:when>
				...
				<tr>
					<td>
					<textarea name="fb_content" id="fb_content" rows="10"	cols="80">${fb.fb_content}</textarea>
					</td>
				</tr>
